...
# CREATE ZONE RECORD
function addRecord() {
    #POST https://api.cloudflare.com/client/v4/zones/d2b4e568601e72a510503a5015eb4c18/dns_records
    RECORDTYPE=$1
    RECORDNAME=$2
    RECORDVALUE=$3
    #
    RETVAL=$($CURL -s -X POST \
        "https://api.cloudflare.com/client/v4/zones/$ZONEID/dns_records" \
        -H "X-Auth-Email: $EMAIL" \
        -H "X-Auth-Key: $GLOBAL_API_KEY" \
        -H "Content-Type: application/json" \
        --data "{\"type\":\"$RECORDTYPE\",\"name\":\"$RECORDNAME\",\"content\":\"$RECORDVALUE\",\"ttl\":120,\"priority\":10,\"proxied\":false}")
    #
    if [[ "$RETVAL" == *'"success":true'* ]]; then
        echo "SUCCESS"
        exit 0
...
function deleteRecord() {
    RECORDID=$1
    #
    RETVAL=$($CURL -s -X DELETE "https://api.cloudflare.com/client/v4/zones/$ZONEID/dns_records/$RECORDID" \
         -H "X-Auth-Email: $EMAIL" \
         -H "X-Auth-Key: $GLOBAL_API_KEY" \
         -H "Content-Type: application/json")
    #
...
case "$1" in
    ...
    ADDRECORD) if [ $# -lt 3 ]
           then
                echo
                echo "  Usage  : $1 <type> <name> <value>"
                echo "  Example: $1 AAAA test.mydomain.org 127.0.0.1"
                echo
                exit -1
            fi
    ...
    DELRECORD) if [ $# -lt 1 ]
           then
                echo
                echo "  Usage  : $1 <id*>"
                echo "  Example: $1 94a30abbca443e03fd11aa00ca74627e"
                echo "      * record id can found by running GETZONE"
                echo
                exit -1
            fi
